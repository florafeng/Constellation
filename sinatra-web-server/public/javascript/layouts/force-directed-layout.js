...
Layout.ForceDirected = function(graph, options) {
  ...
  this.finished = false;
  this.nodePhysics = options.nodePhysics || false;
  ...
  this.init = function() {
    ...
    ac = this.attraction_multiplier * forceConstant;
    rc = this.repulsion_multiplier * forceConstant;
  };
  ...
  this.generate = function() {
    if(layout_iterations < this.max_iterations && temperature > 0.000001) {
      ...
      for(var i=0; i < nodes_length; i++) {
        ...
        var attraction_constant = ac;
        var repulsion_constant = rc;

        if (i == nodes_length - 1) {
          attraction_constant = ac ;
          repulsion_constant = rc * 5;
        } else if (i > 0 && this.nodePhysics) {
          repulsion_constant = rc * (node_v.data.sizeRatio * 10);
        }
